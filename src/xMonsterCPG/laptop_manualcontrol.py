...
# rospy.Subscriber("/hexapod/direction/command", String, command_callback, queue_size=1)
...
while not rospy.is_shutdown():
    ...
    rospy.Subscriber("/m6/joint_states", JointState, joint_callback, queue_size=1)
    timeNow = time.time()
    ...
    #Updating a value
    a = 0.16*np.ones([1, 6])
    # a = np.array([[0.4092], [0.4092], [0.50], [0.50], [0.4092], [0.4092]])/ math.sqrt(2)
